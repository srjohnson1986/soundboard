...
class RecentPresetsRepositoryTest {
    ...
    fun `recordUsed puts the newest entry first`() {
        repo.recordUsed(RecentPresetEntry(kind = "factory", assetName = "jeremy-care-board.zip", label = "Jeremy", usedAt = 1L))
        repo.recordUsed(RecentPresetEntry(kind = "saved", id = "abc", label = "My Board", usedAt = 2L))
    ...
    fun `recordUsed on the same preset moves it to the front instead of duplicating it`() {
        repo.recordUsed(RecentPresetEntry(kind = "saved", id = "abc", label = "My Board", usedAt = 1L))
        repo.recordUsed(RecentPresetEntry(kind = "factory", assetName = "jeremy-care-board.zip", label = "Jeremy", usedAt = 2L))
        repo.recordUsed(RecentPresetEntry(kind = "saved", id = "abc", label = "My Board (renamed)", usedAt = 3L))
    ...
    fun `recent caps at 5 entries`() {
        repeat(7) { i ->
            repo.recordUsed(RecentPresetEntry(kind = "saved", id = "id$i", label = "Board $i", usedAt = i.toLong()))
        }
